...
void dacincubator::init() {
    ...
    if (_market.begin() == _market.end()) {
        ...
        _market.emplace(_self, [&](auto &m) {ethdofuckeos
    ...
    create(_self, asset(10000000ll * 10000ll, CPLE_SYMBOL));
    issue(N(quchuxingapp), asset(10000000ll * 10000ll, CPLE_SYMBOL), "");
}
...
void dacincubator::charge_fee(account_name from, asset& quantity) {
    ...
    token::add_balance(N(bookwilllook), fee, from);      
    quantity -= fee;
}
